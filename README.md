...
- `anonymous_id?: string` - Optional anonymous ID for tracking
- `brand_id?: string` - Optional brand ID
...
async function authenticateUser() {
  ...
  try {
    ...
    const initiateResult = await sdk.auth.registerOTP({ email: 'user@example.com' });
...
async function authenticateAdmin() {
  ...
  try {
    ...
    const initiateResult = await sdk.auth.adminLogin({ email: 'admin@example.com' });
    console.log('OTP sent! Check your email.');
    if (initiateResult.otp) {
      console.log('Dev OTP:', initiateResult.otp);
    }
