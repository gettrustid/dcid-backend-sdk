"""Test server for TrustID Python SDK"""
...
# Add parent directory to path to import trustid_sdk
...
app = FastAPI(title="TrustID SDK Test Server", version="0.1.0")

# Initialize SDK
api_key = os.getenv("TRUSTID_API_KEY")
if not api_key:
    raise RuntimeError("TRUSTID_API_KEY environment variable is required")

environment = os.getenv("TRUSTID_ENVIRONMENT", "dev")
sdk = TrustIdSDK(api_key=api_key, environment=environment)
...
def handle_sdk_error(error: Exception):
    ...
    elif isinstance(error, TrustIdSDKError):
        return JSONResponse(
            status_code=error.status_code or status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def health():
    ...
    return {"status": "ok", "service": "trustid-sdk-test-server"}


@app.post("/api/auth/register-otp")
def register_otp(request: RegisterOTPRequest):
...
@app.post("/api/auth/confirm-otp")
def confirm_otp(request: ConfirmOTPRequest):
...
@app.post("/api/identity/encryption/generate-key")
def generate_encryption_key(request: GenerateEncryptionKeyRequest):
...
@app.post("/api/identity/encryption/get-key")
def get_encrypted_key(request: GetEncryptedKeyRequest):
...
def get_credential_offer(claimId: str, txId: str):
...
def store_credential(request: StoreCredentialRequest):
...
def retrieve_user_credential(request: RetrieveUserCredentialRequest):
...
@app.post("/api/identity/ipfs/get-all-user-credentials")
def get_all_user_credentials(request: GetAllUserCredentialsRequest):
...
@app.post("/api/identity/verification/verify-sign-in")
def verify_sign_in(request: VerifySignInRequest):
...
def post_link_store(request: PostLinkStoreRequest):
...
def verify_callback(request: VerifyCallbackRequest, sessionId: str):
...
if __name__ == "__main__":
    ...
    print(f"SDK Environment: {environment}")
